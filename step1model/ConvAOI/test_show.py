...
from weeplaces.step1model.att_channel_convlstm_64_v2_c6.model2 import *
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            for idx in range(data_batch.shape[0]):
                peo = data_batch[idx, ..., -1].sum()
                if peo > max_peo:
                    counter_idx = idx
                    max_peo = peo
                    max_label_data = data_batch[idx, ...].cpu().numpy()

            max_idx = counter_idx + counter_base

            counter_base += data_batch.shape[0]
            # if 64512 != max_idx:
            #     continue
            predict_batch, cnn_out, rnn_out = preModel(data_batch[:, -1 - CONF['input_len']:-1])
            max_pred_data = predict_batch[counter_idx, ...,].cpu().numpy()
            max_cnn_out = cnn_out[counter_idx, ...].cpu().numpy()
            max_rnn_out = rnn_out[counter_idx, ...].cpu().numpy()
        ...
        np.save('max_label_data.npy', max_label_data)
        np.save('max_pred_data.npy', max_pred_data)
        np.save('max_cnn_out.npy', max_cnn_out)
        np.save('max_rnn_out.npy', max_rnn_out)
        print(max_idx)
        print('end')
